...
class Operator:
    ...
    def get_tbme(self,a,b,c,d,Jab,Jcd):
        ...
        if(self._triag(Jab,Jcd,self.rankJ)): return 0.0
        if(oa.z+ob.z-oc.z-od.z - 2*self.rankZ != 0): return 0.0
        if((-1)**(oa.l + ob.l + oc.l + od.l) * self.rankP != 1): return 0.0
...
def main():
    file_op='snt-file-name'
    rankJ = 0
    rankP = 1
    rankZ = 0
    Op = Operator(file_op, rankJ, rankP, rankZ)
